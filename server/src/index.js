...
app.post("/api/hr/calendar/holiday", requireHrAdmin, async (request,response,next)=>{
  try{
    ...
    return response.json({ok:true,row:saved.rows[0],branch,date,dayType});
  }catch(error){return next(error);}
});
...
app.post("/api/hr/calendar/weekends", requireHrAdmin, async (request,response,next)=>{
  try{
    ...
    const verify=(await query("select weekday,branch from hr_weekend_rules where active=true and lower(branch)=lower($1) order by weekday",[branch])).rows;
    return response.json({ok:true,branch,weekdays, savedWeekdays:verify.map(r=>Number(r.weekday))});
  }catch(error){return next(error);}
});
...
app.put("/api/hr/admin/employee-branch", requireHrAdmin, async (request,response,next)=>{
  try{
    ...
    const saved=await query(`
      insert into employees(user_name,employee_code,full_name,branch,employment_status,created_at,updated_at)
      select u.user_name, '', coalesce(nullif(u.user_name,''), u.email), $1, coalesce(u.account_status,'Active'), now(), now()
      from app_users u
      where lower(u.user_name)=lower($2)
      on conflict(user_name) do update set branch=excluded.branch,updated_at=now()
      returning user_name,employee_code,full_name,branch
    `,[branch,userName]);
    if(!saved.rows[0]) return response.status(404).json({ok:false,error:"Registered employee account not found."});
    return response.json({ok:true,row:saved.rows[0]});
  }catch(error){return next(error);}
});
...
app.get("/api/hr/admin/summary", requireHrAdmin, async (request,response,next)=>{
  try{
    ...
    const employees=(await query(`
      select
        coalesce(e.user_name, u.user_name) as user_name,
        coalesce(e.employee_code, '') as employee_code,
        coalesce(nullif(e.full_name, ''), u.user_name) as full_name,
        coalesce(e.branch, u.branch_access, 'Kuwait HO') as branch,
        coalesce(e.department, '') as department,
        coalesce(e.designation, '') as designation,
        e.join_date,
        coalesce(e.employment_status, u.account_status, 'Active') as employment_status,
        coalesce(e.reporting_manager, '') as reporting_manager
      from app_users u
      left join employees e on lower(e.user_name)=lower(u.user_name)
      where coalesce(u.hr_portal_access, false)=true
        and lower(coalesce(u.role,'')) not in ('admin','hr')
        and lower(coalesce(u.account_status,'active'))='active'
      union
      select user_name,employee_code,full_name,branch,department,designation,join_date,employment_status,reporting_manager
      from employees e
      where not exists (select 1 from app_users u where lower(u.user_name)=lower(e.user_name))
      order by full_name
    `)).rows;
