...
async function findBlockedCustomer(customerName) {
  const lookup = String(customerName || "").trim();
  if (!lookup) return null;
  const result = await query(
    "select code, name, status from customers where lower(name) = lower($1) or lower(code) = lower($1) limit 1",
    [lookup]
  );
  const row = result.rows[0];
  if (row && String(row.status || "").trim().toLowerCase() === "blocked") return row;
  return null;
}
...
app.post("/api/shipments", requireAppAuth, async (request, response, next) => {
  try {
    ...
    const blocked = await findBlockedCustomer(customerName);
    if (blocked) {
