...
async function updateRow(config, id, body) {
  ...
  values.push(id);
  // Customers/suppliers are looked up elsewhere (e.g. findBlockedCustomer) by case-insensitive
  // code/name match, but this update was matching "code" with a plain case-sensitive "=". Any
  // stray case or whitespace difference between what the UI sent and what's stored would make
  // this match zero rows (a silent no-op previously reported back as if it worked), so a
  // block/unblock could keep failing to actually persist. Keep other keys (job_no, load_no, etc.)
  // exact-match, since this mismatch has only been observed for the "code" key.
  const keyComparison = config.key === "code"
    ? `lower(${config.key}) = lower($${values.length})`
    : `${config.key} = $${values.length}`;
  const result = await query(
    `update ${config.table}
...
// branch (from their session), not the customer's branch (set once at customer creation) and not
// necessarily the shipment payload's branch field. A block on "Kuwait HO" should only stop Kuwait
// HO staff from transacting with that customer - Dubai staff are unaffected. Only when the
// logged-in user has "Both" branch access (e.g. admin) do we fall back to whichever branch they
// explicitly picked for this shipment, since their own account isn't tied to one.
// Mirrors the frontend's normalizeBranchName() exactly - user accounts and the request payload can
// both carry either the raw UI label ("Branch 1"/"Branch 2") or the canonical name ("Kuwait
// HO"/"Dubai"); blocked_branches is always stored canonically (set via this same normalization on
// the frontend), so this side of the comparison has to normalize the same way or a raw "Branch 2"
// session value would never match a stored "Dubai" block/non-block, regardless of which is correct
// for that user - the block check would silently never enforce anything.
function normalizeBlockBranchName(value) {
...
function requestBlockCheckBranch(request) {
  const sessionBranch = String(request.appSession?.branchAccess || "").trim();
  if (sessionBranch && !["both", "all"].includes(sessionBranch.toLowerCase())) return normalizeBlockBranchName(sessionBranch);
  return normalizeBlockBranchName(request.body?.branch || "");
}
...
async function findBlockedCustomer(customerName, branch) {
  ...
  if (blockedBranches.length) {
    ...
    const normalizedBranch = normalizeBlockBranchName(branch || "");
...
app.put("/api/shipments/:id", requireAppAuth, async (request, response, next) => {
  try {
    ...
    const blocked = await findBlockedCustomer(customerName, requestBlockCheckBranch(request));
    if (blocked) {
      const error = new Error(`${blocked.name} (${blocked.code}) is blocked for overdue account. Ask admin to approve an unblock request before saving shipments for this customer.`);
      error.status = 409;
      throw error;
    }

    // Global Delivered Shipment Rule: once a shipment's stored status is Delivered, only an
    // Admin session may update it. Mirrors the frontend read-only lock so the rule also holds
    // for direct/API callers, not just the UI.
    const isAdmin = String(request.appSession?.role || "").toLowerCase() === "admin";
    if (!isAdmin) {
      const current = await query("select status from shipments where job_no = $1", [shipmentId]);
      if (current.rows[0] && String(current.rows[0].status || "").trim().toLowerCase() === "delivered") {
        const error = new Error("This shipment is Delivered and read-only. Only an admin can edit it.");
        error.status = 403;
        throw error;
      }
    }
