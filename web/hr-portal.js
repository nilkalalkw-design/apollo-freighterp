...
async function submitLeave(form){
  ...
  try{
    if(selectedType?.require_attachment && file==null){ const start=data.startDate,end=data.endDate; const calc=await hrFetch(`/api/hr/calendar?from=${encodeURIComponent(start)}&to=${encodeURIComponent(end)}`); const s=new Date(`${start}T00:00:00Z`),e=new Date(`${end}T00:00:00Z`); let working=0; const hs=new Map((calc.holidays||[]).map(h=>[String(h.holiday_date).slice(0,10),h])); for(let d=new Date(s);d<=e;d.setUTCDate(d.getUTCDate()+1)){const k=d.toISOString().slice(0,10);if(!calc.weekends.includes(d.getUTCDay())&&!(["PUBLIC_HOLIDAY","BLACKOUT"].includes(hs.get(k)?.day_type)))working++;} if(working>=Number(selectedType.attachment_after_days||0)) throw new Error(`${selectedType.name} requires a supporting document for this leave period.`); }
