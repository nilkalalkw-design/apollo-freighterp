...
function table(type, rows, columns, showLoad = type !== "shipment", scope = type, sortable = true) {
  ...
  return `${scope === "shipment" ? "" : ""}${lockToggle}<div class="table-wrap" data-table-scope="${escapeHtml(scope)}"><table${tableStyle} data-table-scope="${escapeHtml(scope)}"><thead><tr>${headCells}${header}</tr></thead><tbody>${body}</tbody></table></div>`;
}
