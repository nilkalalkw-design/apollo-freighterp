  ...
  const syncSelectedJobs = () => {
    ...
    if (!jobNumbers.length) {
      ...
      const bodyRows = jobNumbers.map((jobNo, index) => {
        ...
        const note = shipmentItem ? "" : `<td class="empty-state">Shipment not found</td>`;
        return `<tr>${cells}${note}<td><button type="button" class="ghost-button" data-remove-consolidation-job="${escapeHtml(jobNo)}">Remove</button></td></tr>`;
      }).join("");
      list.innerHTML = `<div class="table-wrap"><table><thead><tr>${headCells}<th></th><th>Remove</th></tr></thead><tbody>${bodyRows}</tbody></table></div>`;
    }
