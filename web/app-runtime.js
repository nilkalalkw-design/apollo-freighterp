...
function openCustomerShipmentHistory(customerCode, range = {}) {
  ...
  openDialog({
    ...
    body: `<div class="customer-shipment-filter">
      <div class="form-grid">
        <label>From Date<input name="customerShipmentFrom" type="date" value="${escapeHtml(from)}"></label>
        <label>To Date<input name="customerShipmentTo" type="date" value="${escapeHtml(to)}"></label>
      </div>
      <div class="action-row customer-shipment-filter-actions">
        <button type="button" class="primary-button" data-dialog-action="search-customer-shipments">Search</button>
        <button type="button" class="secondary-button" data-dialog-action="clear-customer-shipment-filter">Clear</button>
      </div>
    </div>
    <p class="empty-state">${escapeHtml(String(rows.length))} shipment(s) for ${escapeHtml(customer.name)}.</p>${registerColumnSettingsMarkup("shipment:customer-history", "Customer shipment columns", shipmentColumnDefaults("shipment:customer-history"))}${table("shipment", rows, shipmentColumns("shipment:customer-history"), false, "shipment:customer-history")}`,
    onSave() { recordDialog.close(); },
    onSecondary() { recordDialog.close(); },
    afterOpen() {
      dialogBody.querySelector("[data-dialog-action='search-customer-shipments']")?.addEventListener("click", () => {
        const data = collectFormValues(dialogBody.closest("form"));
        const fromValue = String(data.customerShipmentFrom || "").slice(0, 10);
        const toValue = String(data.customerShipmentTo || "").slice(0, 10);
        if (fromValue && toValue && fromValue > toValue) {
          notifyDenied("Invalid date range", "From Date cannot be after To Date.");
          return;
        }
        openCustomerShipmentHistory(customer.code, { from: fromValue, to: toValue });
      });
      dialogBody.querySelector("[data-dialog-action='clear-customer-shipment-filter']")?.addEventListener("click", () => {
        openCustomerShipmentHistory(customer.code, { from: "", to: "" });
      });
    }
  });
...
function openCustomerDetails(customerCode) {
  ...
  openDialog({
    ...
    body: `
      ...
      <p class="empty-state">${adminActing ? "As admin, block/unblock applies immediately - no approval needed." : "Block/unblock requests are sent to admin and shown in User Management / Settings."}</p>
    `,
    async onSave() {
      const data = collectFormValues(dialogBody.closest("form"));
    ...
    afterOpen: () => {
      ...
      dialogBody.querySelector("[data-dialog-action='block-customer']")?.addEventListener("click", async () => {
        recordDialog.close();
        await submitBlockRequest("customers", customer.code, "Block");
      });
      dialogBody.querySelector("[data-dialog-action='unblock-customer']")?.addEventListener("click", async () => {
        recordDialog.close();
        await submitBlockRequest("customers", customer.code, "Unblock");
      });
    }
  });
}
...
function openRecord(type, id, presetRecord) {
  ...
  dialogBody.innerHTML = type === "shipment"
    ...
    : Object.entries(record)
      ...
      .join("");
  dialogBody.classList.toggle("single-column", type === "shipment");
...
function convertShipmentRequestToShipment(id) {
  ...
  if (denyIfCustomerBlocked(record.customerName)) return;
  editing = null;
  dialogState = null;
...
function blockedCustomerRecord(customerNameOrCode) {
  ...
  return state.customers.find((row) => {
    ...
    return matchesCustomer && String(row.status || "").trim().toLowerCase() === "blocked";
  }) || null;
}

// Shared guard used by every shipment-creation entry point (draft or final). Returns true and
// shows a denial notice if the shipment's customer is currently blocked, so no shipment - draft or
// otherwise - can be created or continued for a blocked account until admin approves an unblock.
function denyIfCustomerBlocked(customerNameOrCode) {
  const blocked = blockedCustomerRecord(customerNameOrCode);
  if (!blocked) return false;
  notifyDenied(
    "Customer blocked",
    `${blocked.name} (${blocked.code}) is blocked for overdue account. Ask admin to approve an unblock request before creating or editing shipments for this customer.`
  );
  return true;
}
...
async function createShipmentDraft(data) {
  if (denyIfCustomerBlocked(data.customer)) return false;
...
async function updateStatus(data) {
  ...
  const oldStatus = String(shipmentItem.status || "").trim();
  const remark = String(data.notes || "").trim();
  const entryDate = String(data.date || "").slice(0, 10);
  const oldExpectedArrivalDate = String(shipmentItem.expectedArrivalDate || "").slice(0, 10);
  if (!newStatus) {
    notifyDenied("Status required", "Select a shipment status before saving.");
    return false;
  }
  if (!/^\d{4}-\d{2}-\d{2}$/.test(entryDate)) {
    notifyDenied("Date required", "Select the date on which this status update actually occurred.");
    return false;
  }
  // Same-status entries are allowed when backfilling a missed journey event. This is important
  // when a shipment has already moved to the next level but the previous status was not recorded.
  if (newStatus.toLowerCase() === oldStatus.toLowerCase() && !remark && entryDate === today() && !data.expectedArrivalDate) {
    notifyDenied("No status change", "Choose a different status, enter a historical date, or add a remark to create a journey update.");
    return false;
  }
  ...
  const originalSnapshot = { ...shipmentItem };
  const updatedShipment = { ...shipmentItem, status: newStatus };
  if (isBranchTransferStatus(newStatus)) {
    updatedShipment.receivingBranch = shipmentReceivingBranch(updatedShipment);
    if (data.expectedArrivalDate) updatedShipment.expectedArrivalDate = String(data.expectedArrivalDate).slice(0, 10);
  }
  updatedShipment.notes = shipmentMetaNotes(updatedShipment);

  const savedShipment = await persistRecord("shipment", updatedShipment);
  if (!savedShipment) {
    Object.assign(shipmentItem, originalSnapshot);
    notifyDenied("Status not saved", "The shipment status could not be saved to the server. Please try again.");
    return false;
  }
  Object.assign(shipmentItem, updatedShipment);
  ...
  if (!historySaved) {
    Object.assign(shipmentItem, originalSnapshot);
    await persistRecord("shipment", shipmentItem);
    notifyDenied("Status history not saved", "The status update was not completed because its history could not be recorded.");
    return false;
  }
  state.shipmentStatusHistory.unshift(historyEntry);

  const statusDetails = `status: ${oldStatus} -> ${newStatus}${remark ? ` | remark: ${remark}` : ""} | date: ${entryDate}${oldExpectedArrivalDate !== String(shipmentItem.expectedArrivalDate || "") ? ` | expected arrival: ${oldExpectedArrivalDate} -> ${String(shipmentItem.expectedArrivalDate || "")}` : shipmentItem.expectedArrivalDate ? ` | expected arrival: ${shipmentItem.expectedArrivalDate}` : ""}`;
  await addHistory("Updated shipment status", `${jobNo} -> ${newStatus}`, statusDetails);
  saveState();
  notifySuccess("Status updated", `${jobNo} is now ${newStatus}. Effective date: ${entryDate}.`);
