...
// A normal (non-admin) user may not edit a shipment once it is Closed. Admins can always edit.
// Requirement 5 (Global Delivered Shipment Rule) restates this without the POD qualifier
// ("Normal users can view only delivered shipments... Admin can edit delivered shipments if
// required") - so read-only is gated on Delivered status alone, not the narrower Closed Job
// (Delivered + POD Uploaded) definition used for the Dashboard card and the POD dropdown filter.
function shipmentIsReadOnlyForCurrentUser(row) { return shipmentStatusIsDelivered(row) && !isAdminSession(); }
...
function openCustomerDetails(customerCode) {
  ...
  openDialog({
    ...
    async onSave() {
      ...
      // Send only the fields this form actually edits - not the whole `customer` object. That
      // object can be stale for blockedBranches/status/isAccountOverdue (block/unblock happens
      // through a separate action, possibly in another tab or minutes earlier), and persistRecord
      // PUTs whatever is in this payload as the new value for every field present. Including the
      // stale block fields here would silently revert a block/unblock that happened after this
      // dialog's data was loaded - which is not what "Save Changes" on name/contact fields should
      // ever do.
      const saved = await persistRecord("customers", {
        code: customer.code,
        name: customer.name,
        locationOrLane: customer.locationOrLane,
  ...
  return `<section class="consolidation-jobs">
    ...
    <div class="status-expand-panel">
      ...
      <form data-form="load-status" class="inline-status-form">
        ...
        ${select("status", "Status", statusOptions(), loadItem.status)}
        ${input("date", "Date", today(), false, "date")}
        ${input("notes", "Manual Remark", "")}
        <div class="action-row">
...
function applyColumnFilters(scope, rows, columns) {
  ...
  return rows.filter((row) => activeEntries.every(([key, term]) => {
    const cellText = String(display(row?.[key]) ?? "").toLowerCase();
...
function podShipmentFields(shipmentItem = {}) {
  return `
    ...
    ${input("deliveryLocation", "Delivery Location", shipmentItem.deliveryLocation || "")}
    ${input("deliveryAddress", "Delivery Address", shipmentItem.deliveryAddress || "")}
...
function bindPodShipmentDialog() {
  ...
  dialogBody.addEventListener("click", async (event) => {
    ...
    if (!saved) return;
    saveState();
    generateRecordDocument("pod", String(data.jobNo || "").trim());
    recordDialog.close();
    render();
  });
}
...
async function savePodDelivery(data) {
  ...
  const updatedShipment = {
    ...
    deliveryLocation: String(data.deliveryLocation || shipmentItem.deliveryLocation || "").trim(),
    deliveryAddress: String(data.deliveryAddress || shipmentItem.deliveryAddress || "").trim(),
    deliveryRemarks: String(data.deliveryRemarks || "").trim(),
    pocName: String(data.pocName || "").trim(),
    pocMobile: String(data.pocMobile || "").trim(),
    additionalContact: String(data.additionalContact || "").trim(),
...
// Looks up the signed POD document already uploaded for a shipment (state.documents entry with
// type "POD" and linkedNo === jobNo) and opens its stored file in a new tab. This is the "View
// POD" action on the POD Pending / Delivery Board - previously there was no way to view an
// uploaded POD directly from that panel.
function findPodDocument(jobNo) {
  return state.documents.find((row) => row.type === "POD" && row.linkedNo === jobNo);
}
...
function viewPodFileForSelectedShipment(jobNo) {
  ...
  const podDocument = findPodDocument(trimmedJobNo);
  if (!podDocument || !podDocument.storageUrl) {
    notifyDenied("POD not available", "No signed POD file has been uploaded for this shipment yet.");
    return;
  }
  window.open(podDocument.storageUrl, "_blank", "noopener");
}
...
function bindShipmentCustomerAutofill() {
  ...
  const fill = (source) => {
    ...
    // Branch-aware, matching the real enforcement in createShipment/createShipmentDraft - a
    // customer blocked only in Kuwait HO should not scare a Dubai user with this warning (it
    // used to check customer.status directly, which goes "Blocked" the moment ANY branch is
    // blocked, so every branch's staff saw this warning regardless of which branch was actually
    // blocked).
    if (isRecordBlockedForBranch(customer, currentBlockCheckBranch(dialogValue("branch")))) {
...
async function submitBlockRequest(type, id, requestType, branch = "") {
  ...
  // Checked against THIS branch specifically (isRecordBlockedForBranch), not the record's overall
  // status - otherwise blocking a customer for a second branch while a first branch's block is
  // already active would be wrongly refused as "already blocked", and unblocking one branch while
  // another stays blocked would be wrongly refused as "not blocked". Shipment block/unblock
  // requests (which don't have a blockedBranches field) fall through to the same plain status
  // check as before, so that path is unaffected.
  const blockedForBranch = isRecordBlockedForBranch(record, chosenBranch);
  if (isBlockAsk && blockedForBranch) {
    notifyDenied("Already blocked", `${displayName} is already blocked${type !== "shipment" ? ` for ${chosenBranch}` : ""}.`);
    return false;
  }
  if (!isBlockAsk && !blockedForBranch) {
    notifyDenied("Not blocked", `${displayName} is not currently blocked${type !== "shipment" ? ` for ${chosenBranch}` : ""}, so there is nothing to unblock.`);
    return false;
  }
  ...
  if (adminActing) {
    ...
    if (!applied) {
      ...
      request.notes = "Database save failed - the record was not actually changed. Try again.";
...
async function approveBlockRequest(request, approvalNotes = "") {
  ...
  const applied = await applyBlockRequestToRecord(request);
  if (!applied) {
    notifyFailed("Approval not applied", `${request.requestType} for ${request.referenceNo || request.customerName} could not be saved to the database. The request is still Pending - try approving again.`);
    return;
  }
...
function currentBlockCheckBranch(explicitBranch) {
  const sessionBranch = String(currentSession()?.branchAccess || "").trim();
  if (sessionBranch && !["both", "all"].includes(sessionBranch.toLowerCase())) {
    const normalized = normalizeBranchName(sessionBranch);
    if (CANONICAL_BRANCHES.includes(normalized)) return normalized;
  }
...
function isRecordBlockedForBranch(record, branch) {
  ...
  if (blockedBranches.length) {
    ...
    const normalizedBranch = normalizeBranchName(branch || "");
    return normalizedBranch ? blockedBranches.includes(normalizedBranch) : blockedBranches.length > 0;
  }
...
async function applyBlockRequestToRecord(request) {
  ...
  const previousIsAccountOverdue = record.isAccountOverdue;
  ...
  // persistRecord() can fail (network hiccup, session issue, a save the server rejects) without
  // throwing - it just returns false and shows its own "Live database save failed" banner. The
  // record above was already updated in memory, so without this check the screen would keep
  // showing the new Active/Blocked status - and every caller of this function would report
  // "success" - even though the database still has the OLD value. That's exactly what let a
  // customer look Unblocked in the UI (and be re-submittable as "already unblocked" over and
  // over) while the database, and therefore server-side enforcement on manifest/shipment saves,
  // never changed. Roll the in-memory record back to what it actually was in the database so the
  // UI stays truthful, and tell the caller the apply did not really happen.
  const saved = await persistRecord(collectionKey, record);
  if (!saved) {
    record.blockedBranches = previousBlockedBranches;
    record.status = previousStatus;
    record.isAccountOverdue = previousIsAccountOverdue;
  }
  return saved;
}
