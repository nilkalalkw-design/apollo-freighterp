...
(function () {
  ...
  const STORAGE_KEY = "apollo.dashboard.layout.v1";
  ...
  function getDashboardContainer() {
    return document.querySelector(
      '[data-page="dashboard"], #dashboard, .dashboard-page, .dashboard-grid, .dashboard-content'
    );
  }
  ...
  function applyDashboardLayout() {
    ...
    items.forEach((el, index) => {
      if (!el.dataset.apolloDashboardId) {
        el.dataset.apolloDashboardId =
          el.id || el.getAttribute("data-key") || `dashboard-item-${index}`;
      }
    });
    ...
    order.forEach(id => {
      const el = items.find(x => x.dataset.apolloDashboardId === id);
      if (el) container.appendChild(el);
    });

    Array.from(container.children).forEach(el => {
      const id = el.dataset.apolloDashboardId;
  ...
  function openDashboardLayoutPanel() {
    const container = getDashboardContainer();
    if (!container) return;

    let panel = document.getElementById("apollo-dashboard-layout-panel");
    if (panel) {
      panel.remove();
      return;
    }
    ...
    panelHost.id = "apollo-dashboard-layout-panel";
    panelHost.style.cssText =
      "position:fixed;right:20px;top:80px;z-index:99999;background:#fff;" +
      "border:1px solid #ccc;border-radius:8px;padding:12px;min-width:260px;" +
      "max-height:70vh;overflow:auto;box-shadow:0 8px 30px rgba(0,0,0,.18);";

    const title = document.createElement("div");
    title.textContent = "Dashboard Layout";
    title.style.cssText = "font-weight:600;margin-bottom:10px;";
    panelHost.appendChild(title);
    ...
    items.forEach(el => {
      ...
      row.style.cssText =
        "display:flex;align-items:center;gap:8px;padding:6px 0;cursor:grab;";
      row.draggable = true;
      row.dataset.id = id;

      const checkbox = document.createElement("input");
      checkbox.type = "checkbox";
      checkbox.checked = !(layout.hidden && layout.hidden[id]);
      checkbox.addEventListener("change", () => {
        const current = readLayout();
        current.hidden = current.hidden || {};
        current.hidden[id] = !checkbox.checked;
        saveLayout(current);
      ...
      row.addEventListener("drop", e => {
        ...
        const sourceEl = items.find(x => x.dataset.apolloDashboardId === fromId);
        const targetEl = items.find(x => x.dataset.apolloDashboardId === id);
        if (sourceEl && targetEl) container.insertBefore(sourceEl, targetEl);
      });
    ...
    actions.style.cssText =
      "display:flex;gap:8px;margin-top:10px;padding-top:10px;border-top:1px solid #eee;";
    ...
    close.addEventListener("click", () => panelHost.remove());

    actions.appendChild(reset);
    actions.appendChild(close);
    panelHost.appendChild(actions);
    document.body.appendChild(panelHost);
  }

  function ensureDashboardControl() {
    const container = getDashboardContainer();
    if (!container) return;

    const existing = document.getElementById("apollo-dashboard-layout-button");
    if (existing) return;

    const button = document.createElement("button");
    button.id = "apollo-dashboard-layout-button";
    button.type = "button";
    button.textContent = "⚙ Dashboard Layout";
    button.title = "Show, hide and arrange dashboard items";
    button.addEventListener("click", openDashboardLayoutPanel);

    const host =
      container.parentElement?.querySelector(".dashboard-header, .page-header, .toolbar") ||
      container.parentElement;
    if (host) host.prepend(button);
  }

  function initDashboardLayoutFeature() {
    applyDashboardLayout();
    ensureDashboardControl();
  }

  window.apolloDashboardLayout = {
    init: initDashboardLayoutFeature,
    open: openDashboardLayoutPanel,
    reset: () => {
      localStorage.removeItem(STORAGE_KEY);
      location.reload();
    }
  ...
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", initDashboardLayoutFeature);
  } else {
    initDashboardLayoutFeature();
  }
