  ...
  const rows = splits.map((split) => {
    ...
    return `
      <tr>
        ...
        <td>${podDoc?.storageUrl ? `<button type="button" class="table-inline-link" data-pod-split-view="${escapeHtml(podDoc.storageUrl)}">View POD</button>` : `<span class="empty-state">No file</span>`}</td>
      </tr>`;
  }).join("");
  return `
    <section class="pod-split-history">
      ...
      <div class="table-wrap"><table>
        <thead><tr><th>#</th><th>Pieces</th><th>Location / Address</th><th>Date</th><th>Received By</th><th>POD</th></tr></thead>
        <tbody>${rows}</tbody>
      </table></div>
    </section>`;
}
