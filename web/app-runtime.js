  ...
  return `<section class="panel">${panelHeader("My Profile")}
    <form class="stack-form" data-form="employee-profile">
      ...
      <div class="detail-grid">
        ...
        ${input("branch", "Branch", myRecord.branch || "Kuwait HO", true)}
...
async function updateEmployeeProfile(data) {
  ...
  const record = {
    ...
    branch: existing?.branch || "Kuwait HO",
...
(function () {
  ...
  // Watch for replacement of the dashboard root, but ignore DOM mutations
  // caused by our own dashboard layout updates. The old document-wide observer
  // called init() after every child mutation and could repeatedly move dashboard
  // nodes, starving the browser when another portal was being rendered.
  const layoutObserver = new MutationObserver((mutations) => {
    const currentRoot = root();
    if (!currentRoot) return;
    const externalMutation = mutations.some((mutation) => {
      const target = mutation.target;
      return target !== currentRoot && !currentRoot.contains(target);
    });
    if (externalMutation) init();
  });
