...
function statusOptions() {
  return dropdownOptions("status", ["Draft", "Booked", "Dispatched", "In-Transit", "Partially Delivered", "Delivered", "Invoiced", "Closed", "Blocked"]);
}
...
function podShipmentFields(shipmentItem = {}) {
  ...
  return `
    ...
    ${input("deliveryAddress", "Delivery Address", "")}
  ...
  const rows = splits.map((split) => {
    ...
    return `
      <tr>
        ...
        <td>${escapeHtml(split.deliveryDatetime ? new Date(split.deliveryDatetime).toLocaleString() : "-")}</td>
        <td>${escapeHtml(split.receivedBy || "-")}</td>
        <td>
          <button type="button" class="table-inline-link" data-pod-split-note data-job-no="${escapeHtml(shipmentItem.jobNo)}" data-split-no="${escapeHtml(String(split.splitNo || ""))}">Delivery Note (Part ${escapeHtml(String(split.splitNo || ""))})</button><br />
          ${podDoc?.storageUrl ? `<button type="button" class="table-inline-link" data-pod-split-view="${escapeHtml(podDoc.storageUrl)}">Signed POD File</button>` : `<span class="empty-state">No signed file uploaded</span>`}
        </td>
      </tr>`;
  }).join("");
  return `
    <section class="pod-split-history">
      ...
      <p class="empty-state">Each delivery has its own printable Delivery Note (regenerate it any time below) and, once uploaded, its own signed POD file.</p>
      <div class="table-wrap"><table>
        <thead><tr><th>#</th><th>Pieces</th><th>Location / Address</th><th>Date</th><th>Received By</th><th>Documents</th></tr></thead>
        <tbody>${rows}</tbody>
      </table></div>
    </section>`;
}
...
function bindPodShipmentDialog() {
  ...
  dialogBody.addEventListener("click", async (event) => {
    ...
    if (noteButton) {
      const noteJobNo = noteButton.dataset.jobNo;
      const noteSplitNo = Number(noteButton.dataset.splitNo);
      const noteShipment = state.shipments.find((row) => row.jobNo === noteJobNo);
      const noteSplit = parsePodSplits(noteShipment?.podSplitsJson).find((item) => Number(item.splitNo) === noteSplitNo);
      if (!noteShipment || !noteSplit) {
        notifyDenied("Delivery not found", "That delivery could not be found - refresh and try again.");
        return;
      }
      openPrintableDocument(podSplitDocumentHtml(noteShipment, noteSplit));
      return;
    }
...
async function uploadPodFileForSelectedShipment(jobNo) {
  ...
  fileInput.addEventListener("change", async () => {
    ...
    if (!file) return;
    try {
      const documentItem = await uploadPodDocument(shipmentItem.jobNo, file, latestSplit ? latestSplit.splitNo : "");
      if (latestSplit) {
        latestSplit.documentNo = documentItem.documentNo;
        latestSplit.fileName = documentItem.fileName;
...
function podSplitDocumentHtml(record, split) {
  ...
  const totalDeliveriesRecorded = parsePodSplits(record.podSplitsJson).length || 1;
  const deliveryNo = split.deliveryNoteNo || `POD-${record.jobNo}${split.splitNo ? `-${split.splitNo}` : ""}`;
  const piecesLabel = totalPieces
    ? `${splitPieces} of ${totalPieces} total piece(s)${isPartial ? ` - Delivery ${split.splitNo || 1}` : ""}`
    : String(splitPieces || record.pieces || 0);
  // Always shown (not just when partial) so every printed POD states its own position in the
  // sequence - e.g. "Delivery 2 of 2" - even once the shipment is fully delivered, so a batch of
  // PODs for one job is never ambiguous about which delivery each page covers.
  const sequenceLabel = `Delivery ${split.splitNo || 1} of ${totalDeliveriesRecorded}`;
  return documentShell(
    `POD ${deliveryNo}`,
    ...
    `
      <section class="document-summary">
        ...
        <div style="grid-column: 1 / -1;"><span>Delivery Sequence</span><strong>${escapeHtml(sequenceLabel)}</strong><small>${totalDeliveriesRecorded > 1 ? "This shipment is delivered in multiple parts" : "Single delivery"}</small></div>
      </section>
      ...
      <section class="delivery-signatures">
        <div><span>Prepared By</span><strong>${escapeHtml(split.preparedBy || currentUserName())}</strong><small>Date & Time</small><em>${escapeHtml(split.deliveryDatetime || new Date().toLocaleString())}</em></div>
