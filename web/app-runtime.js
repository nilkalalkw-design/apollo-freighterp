...
async function applyBlockRequestToRecord(request) {
  ...
  if (record) {
    ...
    // Track which branch(es) this block/unblock applies to, in addition to the existing overall
    // status/isAccountOverdue flags above (kept unchanged so every existing block-check - shipment
    // save guards, etc. - keeps working exactly as before). "Both" always replaces the whole set;
    // a single branch is added to or removed from the existing blocked-branch list.
