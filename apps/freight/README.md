...
The Render web service start command only starts the API. This keeps `/api/health` available even if the database has a connection or migration problem. Run migrations/admin seed as a one-off job or from a trusted local shell:
